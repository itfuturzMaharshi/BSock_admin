...
export interface SellerProductPermissionResponse {
  status: number;
  message: string;
  data: SellerProductPermission;
}
...
export class SellerProductPermissionService {
  ...
  // Get seller product permissions
  static getSellerProductPermissions = async (sellerId?: string): Promise<SellerProductPermission> => {
    const baseUrl = import.meta.env.VITE_BASE_URL;
    const adminRoute = import.meta.env.VITE_ADMIN_ROUTE;
    const url = `${baseUrl}/api/${adminRoute}/seller-product-permission/get`;

    try {
      const res = await api.post(url, { sellerId: sellerId || null });
      if (res.data?.status !== 200) {
        throw new Error(res.data?.message || 'Failed to fetch seller product permissions');
      }
      return res.data.data;
    } catch (err: any) {
      const errorMessage = err.response?.data?.message || 'Failed to fetch seller product permissions';
      // Don't show toast on initial load if no permissions exist yet
  ...
  // Update seller product permissions
  static updateSellerProductPermissions = async (
    permissions: SellerProductFieldPermission[],
    sellerId?: string
  ): Promise<SellerProductPermissionResponse> => {
    const baseUrl = import.meta.env.VITE_BASE_URL;
    const adminRoute = import.meta.env.VITE_ADMIN_ROUTE;
    const url = `${baseUrl}/api/${adminRoute}/seller-product-permission/update`;
    ...
    try {
      ...
      if (res.data?.status !== 200) {
        throw new Error(res.data?.message || 'Failed to update seller product permissions');
      }
      toastHelper.showTost(res.data.message || 'Seller product permissions updated successfully!', 'success');
      return res.data;
    } catch (err: any) {
      const errorMessage = err.response?.data?.message || 'Failed to update seller product permissions';
      toastHelper.showTost(errorMessage, 'error');
      throw new Error(errorMessage);
    }
  };
  ...
  static getCurrentSellerPermissions = async (): Promise<SellerProductFieldPermission[]> => {
    ...
    const url = `${baseUrl}/api/${sellerRoute}/product-permission/get`;
