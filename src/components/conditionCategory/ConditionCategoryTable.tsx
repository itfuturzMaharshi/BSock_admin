...
const ConditionCategoryTable: React.FC = () => {
  ...
  const handleSequenceClick = (item: ConditionCategory) => {
    if (!item._id) return;
  ...
  const handleSequenceSave = async (item: ConditionCategory) => {
    if (!item._id) return;
    ...
    try {
      await ConditionCategoryService.updateSequence(item._id, newSequence);
      setEditingSequenceId(null);
      setEditingSequenceValue("");
      await fetchConditionCategories();
    } catch (error: any) {
        ...
        <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              onClick={async () => {
                try {
                  await ConditionCategoryService.downloadSample();
                } catch (error) {
            ...
            <label className="inline-flex items-center gap-1 rounded-lg bg-orange-600 text-white px-4 py-2 text-sm font-medium hover:bg-orange-700 dark:bg-orange-500 dark:hover:bg-orange-600 transition-colors cursor-pointer">
              ...
              <input
                ...
                onChange={async (e) => {
                  ...
                  if (file) {
                    try {
                      await ConditionCategoryService.importFromExcel(file);
                      fetchConditionCategories();
                    } catch (error) {
