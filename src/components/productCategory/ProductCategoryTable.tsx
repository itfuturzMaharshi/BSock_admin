...
const ProductCategoryTable: React.FC = () => {
  ...
  const handleSave = async (categoryData: any) => {
    try {
      if (editCategory && editCategory._id) {
        await ProductCategoryService.updateProductCategory(editCategory._id, categoryData);
        toastHelper.showTost("Product Category updated successfully!", "success");
      } else {
        await ProductCategoryService.createProductCategory(categoryData);
        toastHelper.showTost("Product Category added successfully!", "success");
      }
  ...
  const handleDelete = async (category: ProductCategory) => {
    ...
    if (confirmed.isConfirmed) {
      try {
        await ProductCategoryService.deleteProductCategory(category._id);
        toastHelper.showTost("Product Category deleted successfully!", "success");
        fetchCategories();
      } catch (error) {
        ...
        <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              onClick={async () => {
                try {
                  await ProductCategoryService.downloadSample();
                } catch (error) {
            ...
            <button
              ...
              onClick={async () => {
                try {
                  await ProductCategoryService.exportToExcel();
                  fetchCategories();
                } catch (error) {
            ...
            <label className="inline-flex items-center gap-1 rounded-lg bg-orange-600 text-white px-4 py-2 text-sm font-medium hover:bg-orange-700 dark:bg-orange-500 dark:hover:bg-orange-600 transition-colors cursor-pointer">
              ...
              <input
                ...
                onChange={async (e) => {
                  ...
                  if (file) {
                    try {
                      await ProductCategoryService.importFromExcel(file);
                      fetchCategories();
                    } catch (error) {
          ...
          <table className="w-full table-auto">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {loading ? (
                ...
                categoriesData.map((item: ProductCategory, index: number) => (
                  <tr
                    key={item._id || index}
